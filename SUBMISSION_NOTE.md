...
The Streamlit interface is organized around the investor narrative rather than developer controls. The visible flow starts with the pipeline: Manifest list, first screen, homepage evidence, search enrichment, and AI scoring. It then shows cost and usage, ranked prospects, and visible evidence-routing counts. There is no sidebar, no tabbed dashboard, no hidden expanders, and no tuning controls in the main workflow. The goal is for the interviewer to understand the filtering logic, evidence quality, and cost discipline without first navigating charts, settings, or implementation details.
...
For the live demo, the app is intentionally focused on the core investor workflow instead of developer controls. The visible surface is the pipeline, the cost and usage report, ranked prospects, and evidence-routing counts. The intended narrative is simple: the tool starts with the Manifest list; removes obvious non-prospects; gathers cheap homepage evidence first; escalates to Tavily only when evidence is missing or unclear; sends compact evidence packets to OpenAI for scoring; and reports actual billed cost, tokens, search credits, and paid calls avoided.
