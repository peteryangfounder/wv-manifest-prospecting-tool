...
The Streamlit interface is organized like a short onboarding deck rather than a dashboard. Each screen teaches one idea: overview, source screening, homepage evidence, paid-run estimate, cost reporting, ranked prospects, and evidence routing. There is no sidebar, no tabbed dashboard, no hidden expanders, and no tuning controls in the main workflow. The investor moves with Previous and Next, sees progress at the top, and only encounters action buttons when a real workflow action is available.
...
The current provider architecture is optimized for network-bound I/O. It should scale materially better than serial processing for thousands of rows, but the correct concurrency settings still depend on provider rate limits, account tier, and acceptable spend. The app therefore shows estimated cost and runtime before execution instead of hiding the operational impact.
...
For the live demo, the app is intentionally focused on the core investor workflow instead of developer controls. The visible surface is a slide-by-slide walkthrough of the pipeline, cost and usage, ranked prospects, and evidence-routing counts. The intended narrative is simple: the tool starts with the Manifest list; removes obvious non-prospects; gathers cheap homepage evidence first; escalates to Tavily only when evidence is missing or unclear; sends compact evidence packets to OpenAI for scoring; and reports actual billed cost, tokens, search credits, and paid calls avoided.
